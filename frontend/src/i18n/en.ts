const en = {
  common: {
    loading: 'Loading...',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    confirm: 'Confirm',
    create: 'Create',
    edit: 'Edit',
    search: 'Search',
    prev: 'Prev',
    next: 'Next',
    active: 'Active',
    disabled: 'Disabled',
    actions: 'Actions',
    comingSoon: 'Coming soon...',
    noData: 'No data',
    ok: 'OK',
  },
  auth: {
    login: 'Login',
    register: 'Register',
    username: 'Username',
    password: 'Password',
    email: 'Email',
    signIn: 'Sign In',
    signingIn: 'Signing in...',
    createAccount: 'Create Account',
    creatingAccount: 'Creating account...',
    noAccount: "Don't have an account?",
    hasAccount: 'Already have an account?',
    brandName: 'COOL API',
    brandDesc: 'AI API Relay Station',
    usernamePlaceholder: 'Enter username',
    passwordPlaceholder: 'Enter password',
    emailPlaceholder: 'you@example.com',
    usernameHint: '3-64 characters',
    passwordHint: 'At least 6 characters',
    invalidCredentials: 'Incorrect username or password',
    loginFailed: 'Login failed, please try again',
  },
  nav: {
    dashboard: 'Dashboard',
    users: 'Users',
    tokens: 'Tokens',
    providerKeys: 'Provider Keys',
    channels: 'Channels',
    rateLimits: 'Rate Limits',
    logs: 'Logs',
    billing: 'Billing',
    pricing: 'Pricing',
    settings: 'Settings',
    apiKeys: 'API Keys',
    usage: 'Usage',
    docs: 'Docs',
    playground: 'Playground',
    adminPanel: 'Admin Panel',
    clientPortal: 'Client Portal',
    collapse: 'Collapse',
    logout: 'Logout',
  },
  admin: {
    dashboard: {
      title: 'Dashboard',
      todayRequests: "Today's Requests",
      todayCost: "Today's Cost",
      activeTokens: 'Active Tokens',
      onlineUsers: 'Online Now',
      requestTrend: 'Request Trend',
      costTrend: 'Cost Trend',
      days7: '7D',
      days30: '30D',
      modelRanking: 'Model Usage Ranking',
      recentRequests: 'Recent Requests',
      cacheHitRate: 'Cache Hit Rate',
      cacheRead: 'Cache Read',
      cacheCreation: 'Cache Creation',
      requests: 'Requests',
      allGroups: 'All Groups',
      model: 'Model',
      tokens: 'Tokens',
      cost: 'Cost',
      status: 'Status',
      time: 'Time',
      noData: 'No data',
    },
    users: {
      title: 'Users',
      searchPlaceholder: 'Search users...',
      username: 'Username',
      email: 'Email',
      role: 'Role',
      balance: 'Balance',
      status: 'Status',
    },
    keys: {
      title: 'Provider Keys',
      addKey: 'Add Key',
      name: 'Name',
      apiKey: 'API Key',
      baseUrl: 'Base URL (optional)',
      weight: 'Weight',
      priority: 'Priority',
      creating: 'Creating...',
      createKey: 'Create Key',
      noKeys: 'No {provider} keys configured',
      deleteConfirm: 'Delete this key?',
      fetchModels: 'Fetch Models',
      fetching: 'Fetching...',
      fetchError: 'Failed to fetch models',
      modelsFound: '{{count}} models found',
      selectAll: 'Select All',
      deselectAll: 'Deselect All',
      selectedModels: 'Selected Models',
    },
    channels: {
      title: 'Channels',
      addChannel: 'Add Channel',
      name: 'Name',
      modelPattern: 'Model Pattern',
      strategy: 'Strategy',
      providerKeys: 'Provider Keys',
      noChannels: 'No channels configured',
      deleteConfirm: 'Delete this channel?',
      creating: 'Creating...',
      createChannel: 'Create Channel',
      editChannel: 'Edit Channel',
      models: 'Models',
      addModelPlaceholder: 'Type model name and press Enter',
      fetchModels: 'Fetch Models',
      fetching: 'Fetching...',
      fetchError: 'Failed to fetch',
      selectModels: 'Select Models',
      provider: 'Provider',
      newCredentials: 'New Credentials',
      existingKeys: 'Existing Keys',
      noModelsSelected: 'Please fetch and select models first',
    },
    billing: {
      title: 'Billing Management',
      topup: 'Topup',
      topupTitle: 'Topup User Balance',
      userId: 'Username',
      amount: 'Amount ($)',
      description: 'Description',
      processing: 'Processing...',
      topupSuccess: 'Topup successful',
      type: 'Type',
      amountCol: 'Amount',
      balanceAfter: 'Balance After',
      time: 'Time',
      noTransactions: 'No transactions',
    },
    settings: {
      title: 'System Settings',
      addKey: 'Add Key',
      saving: 'Saving...',
      saved: 'Settings saved',
      noSettings: 'No settings configured. Click "Add Key" to create one.',
      keyPrompt: 'Setting key:',
    },
    pricing: {
      title: 'Model Pricing',
      syncBtn: 'Sync Provider Prices',
      syncing: 'Syncing...',
      syncResult: 'Synced: {{added}} added, {{updated}} updated, {{total}} total',
      model: 'Model',
      provider: 'Provider',
      inputPrice: 'Input Price',
      outputPrice: 'Output Price',
      multiplier: 'Multiplier',
      effectiveInput: 'Effective Input',
      effectiveOutput: 'Effective Output',
      status: 'Status',
      selectedCount: '{{count}} selected',
      batchSet: 'Batch Set Multiplier',
      empty: 'No pricing data. Click "Sync Provider Prices" to import.',
      deleteConfirm: 'Are you sure?',
      groups: 'Pricing Groups',
      addGroup: 'Add Group',
      editGroup: 'Edit Group',
      noGroups: 'No groups yet. Click "Add Group" to create one.',
      groupName: 'Group Name',
      groupNamePlaceholder: 'e.g. Basic, Pro',
      selectChannels: 'Select Channels',
      channelsCount: 'channels',
    },
    tokens: {
      title: 'Token Management',
      total: '{{count}} tokens',
      noTokens: 'No tokens',
      deleteConfirm: 'Delete this token?',
      createToken: 'Create Token',
      name: 'Token Name',
      namePlaceholder: 'e.g. My App',
      group: 'Group',
      noGroup: 'No group',
      remark: 'Remark',
      remarkPlaceholder: 'Optional remark',
      creating: 'Creating...',
      copyWarning: 'Copy this key now. It won\'t be shown again.',
    },
    logs: {
      title: 'Request Logs',
      time: 'Time',
      model: 'Model',
      status: 'Status',
      tokens: 'Tokens',
      cost: 'Cost',
      latency: 'Latency',
      stream: 'Stream',
      error: 'Error',
      sync: 'Sync',
      noLogs: 'No logs yet',
    },
    rateLimits: {
      title: 'Rate Limit Management',
      globalConfig: 'Global Configuration',
      defaultUserLimit: 'Default User Limit',
      globalLimit: 'Global Limit',
      notSet: 'Not Set',
      envConfig: 'Environment Variable',
      configNote: 'To modify global settings, update DEFAULT_USER_RPM_LIMIT and GLOBAL_RPM_LIMIT in your .env file and restart the service.',
      configEditNote: 'Click Edit to modify global rate limit settings. Changes take effect immediately.',
      edit: 'Edit',
      optional: 'Optional (leave empty for no limit)',
      saving: 'Saving...',
      invalidDefaultUserRpm: 'Default user RPM must be a positive number',
      invalidGlobalRpm: 'Global RPM must be a positive number or empty',
      configUpdateFailed: 'Failed to update configuration',
      infoTitle: 'Rate Limiting Strategy',
      infoDesc: 'This system uses a 3-tier rate limiting approach: Global → User → API Key. Users with null limits use the default (60 RPM). Set custom limits per user or leave empty for default.',
      totalUsers: 'Total Users',
      withLimits: 'With Custom Limits',
      unlimited: 'Using Default',
      avgLimit: 'Avg Custom Limit',
      userRateLimits: 'User Rate Limits',
      searchPlaceholder: 'Search users...',
      username: 'Username',
      email: 'Email',
      role: 'Role',
      status: 'Status',
      rpmLimit: 'RPM Limit',
      actions: 'Actions',
      active: 'Active',
      inactive: 'Inactive',
      defaultLimit: 'Default (60)',
      rpmSuffix: 'RPM',
      editLimit: 'Edit rate limit',
      save: 'Save',
      cancel: 'Cancel',
      noUsers: 'No users found',
      defaultPlaceholder: 'Default (60)',
      invalidNumber: 'Please enter a valid positive number or leave empty for unlimited',
      updateFailed: 'Failed to update rate limit',
      selectedCount: '{{count}} selected',
      batchUpdate: 'Batch Update',
      batchUpdateDesc: 'Set rate limit for {{count}} selected users',
      newRpmLimit: 'New RPM Limit',
      batchUpdateHint: 'Leave empty to reset to default, or enter a number',
      noUsersSelected: 'Please select at least one user',
      batchUpdateFailed: 'Failed to batch update rate limits',
      updating: 'Updating...',
      update: 'Update',
    },
  },
  client: {
    dashboard: {
      welcome: 'Welcome back, ',
      balance: 'Balance',
      apiKeys: 'API Keys',
      recentTokens: 'Recent Tokens',
      recentCost: 'Recent Cost',
      recentRequests: 'Recent Requests',
      model: 'Model',
      tokens: 'Tokens',
      cost: 'Cost',
      time: 'Time',
      noRequests: 'No requests yet. Generate an API key and start making requests.',
    },
    keys: {
      title: 'API Keys',
      generateNew: 'Generate New Key',
      keyName: 'Key Name',
      keyNamePlaceholder: 'Key name (e.g. My App)',
      creating: 'Creating...',
      copyWarning: 'Copy this key now. It won\'t be shown again.',
      noKeys: 'No API keys yet. Click to create one.',
      revokeConfirm: 'Revoke this key? This cannot be undone.',
      created: 'Created',
      noGroup: 'No group',
      group: 'Group',
      remark: 'Remark',
      remarkPlaceholder: 'Optional remark',
    },
    usage: {
      title: 'Usage Statistics',
      pageTotalCost: 'Page Total Cost',
      pageTotalTokens: 'Page Total Tokens',
      time: 'Time',
      model: 'Model',
      status: 'Status',
      tokens: 'Tokens',
      cost: 'Cost',
      latency: 'Latency',
      noUsage: 'No usage yet',
    },
    billing: {
      title: 'Billing',
      currentBalance: 'Current Balance',
      recentTransactions: 'Recent Transactions',
      type: 'Type',
      amount: 'Amount',
      balance: 'Balance',
      description: 'Description',
      time: 'Time',
      noTransactions: 'No transactions yet',
    },
    docs: {
      title: 'API Documentation',
      baseUrl: 'Base URL',
      authentication: 'Authentication',
      authenticationDesc: 'All API requests require a relay key in the Authorization header.',
      chatCompletions: 'Chat Completions',
      chatCompletionsDesc: 'Compatible with the OpenAI Chat Completions API format.',
      streaming: 'Streaming',
      streamingDesc: 'Set stream to true to receive Server-Sent Events.',
      listModels: 'List Models',
      sdkCompatibility: 'SDK Compatibility',
      sdkCompatibilityDesc: 'Use any OpenAI-compatible SDK by changing the base URL.',
    },
    playground: {
      title: 'Playground',
      apiKeyPlaceholder: 'sk-cool-your-key',
      messagePlaceholder: 'Type a message...',
      startChat: 'Send a message to start chatting',
    },
  },
  home: {
    hero: {
      title: 'COOL API',
      subtitle: 'AI API Relay Station',
      description: 'Unified access to OpenAI, Claude, Gemini and more. One key for all models.',
      login: 'Login',
      register: 'Get Started Free',
      console: 'Go to Console',
      pricing: 'View Pricing',
    },
    features: {
      title: 'Why COOL API',
      multiModel: 'Multi-Model Support',
      multiModelDesc: 'Unified access to OpenAI, Claude, Gemini and more. No need to manage multiple API keys.',
      streaming: 'Streaming Output',
      streamingDesc: 'Full SSE streaming support. Real-time model output, identical to official APIs.',
      billing: 'Precise Billing',
      billingDesc: 'Per-token billing with real-time usage and balance tracking. Transparent and controllable.',
      compatible: 'OpenAI Compatible',
      compatibleDesc: 'Fully compatible with OpenAI API format. Zero code changes needed. Works with all major SDKs.',
    },
    models: {
      title: 'Supported Models',
      subtitle: 'Smart channel routing for optimal performance',
      all: 'All',
      channel: 'Channel',
      strategy: 'Strategy',
      noModels: 'No models configured yet. Contact admin.',
    },
    pricing: {
      title: 'Model Pricing',
      subtitle: 'Per-token billing, transparent and open',
      backHome: 'Back to Home',
      model: 'Model',
      provider: 'Provider',
      inputPrice: 'Input Price',
      outputPrice: 'Output Price',
      unit: '/ 1M Tokens',
    },
    cta: {
      title: 'Get Started',
      description: 'Sign up, get your API key, and start using world-class AI models instantly.',
      register: 'Sign Up Now',
    },
  },
};

export default en;
